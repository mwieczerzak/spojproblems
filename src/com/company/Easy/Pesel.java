...
/**
 * JPESEL - Pesel
 Jan Kowalski musi wpisać do systemu szpitalnego dane osobowe pacjenta, oprócz imienia i nazwiska
 musi również wpisać PESEL pacjenta. Jakież było jego zdziwienie, gdy spostrzegł, że pewnych pacjentów
 ...
 Jeśli suma jest większa od zera, wtedy sprawdzamy jej poprawność. W przeciwnym przypadku nr PESEL jest błędny.
 Ponieważ ostatnia cyfra liczby 110 jest zerem więc podany PESEL jest prawidłowy.

 Na wejściu podana jest w pojedyńczej linii ilość t<=100 numerów PESEL do sprawdzenia.
 W kolejnych t liniach są 11-cyfrowe liczby.
...
public class Pesel {
    ...
    public static void main(String[] vargs) throws IOException {
        ...
        int liczbaTestow = Integer.valueOf(r.readLine());

        for (int i = 0; i < liczbaTestow; i++) {
            String pesel = r.readLine();
            int[] mnozniki = {1, 3, 7, 9, 1, 3, 7, 9, 1, 3, 1};
            Integer sumaCyfr = 0;

            for (int j = 0; j < 11; j++) {
                sumaCyfr += Integer.parseInt(pesel.substring(j, j + 1)) * mnozniki[j];
            }

            if (sumaCyfr > 0 && sumaCyfr.toString().endsWith("0")) {
